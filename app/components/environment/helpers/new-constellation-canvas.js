...
export function addConstellationCanvas(scene){
...
var createCSSobject = function(mesh, scene, videoID, renderer) {
    ...
    iframe.src = [ 'https://www.vsynctester.com/testing/mouse.html', '?rel=0&enablejsapi=1&disablekb=1&autoplay=1&controls=0&fs=0&modestbranding=1' ].join( '' )
...
function CreateBackgroundMeshCollider( scene )
{

    if( iframe_bg_collider == null )
    {
        iframe_bg_collider = BABYLON.MeshBuilder.CreateSphere( "iframe-bg-collider", { diameter: 1 }, scene );
        //iframe_bg_collider.setParent( iframe_manager_node );
        iframe_bg_collider.checkCollisions = true;
        iframe_bg_collider.renderOverlay = true;
        iframe_bg_collider.position = new BABYLON.Vector3( 0, 0, 0 );
        iframe_bg_collider.scaling = new BABYLON.Vector3( 100, 100, 100 );
    }
}

    // The CSS object will follow this mesh
    const plane = BABYLON.MeshBuilder.CreatePlane("youtube", {width: 1, height: 1}, scene);
    ...
    var listener = function(evt) 
    {
        ...
        if (pick.hit) 
        {
            ...
            else if( pick.pickedMesh.name === "iframe-bg-collider" )
            {
                if( youtubeFocused )
                {
                    youtubeFocused = false;
    ...
    buildGUI(plane);
    CreateBackgroundMeshCollider(scene);
    return scene;
};
