...
function handleStoreEvent(editor, change, collection){
    if(change.changes.added){
        for(const record of Object.values(change.changes.added)){
            if(record.typeName === 'shape' && (['arrow', 'concept', 'excerpt'].includes(record.type) || (record.type === 'geo' && record.props.geo === 'rectangle'))){
...
export function GraphTrigger(){
    ...
    useLayoutEffect(()=>{
        const relevantShapes = editor.getCurrentPageShapes().filter(shape => ['arrow', 'concept', 'excerpt'].includes(shape.type) || shape.type === 'geo' && shape.props.geo === 'rectangle')
