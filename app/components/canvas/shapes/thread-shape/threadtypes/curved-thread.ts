...
import { TLArcInfo, TLThreadInfo } from './threadtypes'
import {
	BOUND_ARROW_OFFSET,
	MIN_ARROW_LENGTH,
	STROKE_SIZES,
...
import { getStraightThreadInfo } from './straight-thread'

export function getCurvedThreadInfo(
	editor: Editor,
	shape: TLThreadShape,
	bindings: TLThreadBindings
): TLThreadInfo {
	const { threadheadEnd, threadheadStart } = shape.props
	...
	if (
		...
		return getStraightThreadInfo(editor, shape, bindings)
	}

	const terminalsInThreadSpace = getThreadTerminalsInThreadSpace(editor, shape, bindings)

	const med = Vec.Med(terminalsInThreadSpace.start, terminalsInThreadSpace.end) // point between start and end
	const distance = Vec.Sub(terminalsInThreadSpace.end, terminalsInThreadSpace.start)
	...
	// The positions of the body of the thread, which may be different
	// than the thread's start / end points if the thread is bound to shapes
	...
	if (startShapeInfo && !startShapeInfo.isExact) {
		const startInPageSpace = Mat.applyToPoint(threadPageTransform, tempA)
		const centerInPageSpace = Mat.applyToPoint(threadPageTransform, handleArc.center)
		const endInPageSpace = Mat.applyToPoint(threadPageTransform, tempB)
